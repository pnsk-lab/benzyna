...
static char* tts_darray_getDataInPos(struct tts_darray_t* darray, int aPosition) {
	...
	if(aPosition >= darray->mAllocated) {
		...
		newdata = (char*)SPEECH_MALLOC(darray->memctx, newsize);
		if(!newdata) {
...
static void tts_klatt_parwave(struct tts_klatt_t* klatt, struct tts_klatt_frame_t* frame, short int* jwave) {
	...
	for(ns = 0; ns < klatt->mNspFr; ns++) {
		...
		*/
		...
		out = tts_resonator_resonate(&klatt->mParallelFormant1, par_glotout);
...
short* speech_gen(int* samples_pairs_generated, char const* str, void* memctx) {
	...
	frames = tts_phone_to_elm(tts_darray_getData(&phone), tts_darray_getSize(&phone), &element);

	tts_klatt_init(&synth);

	sample_count = synth.mNspFr * frames;

#if 1
	if(samples_pairs_generated) *samples_pairs_generated = sample_count;
	sample_pairs = (short*)SPEECH_MALLOC(memctx, sample_count * sizeof(short));
	...
	sample_pairs = (short*)SPEECH_MALLOC(memctx, sample_count * 8 * sizeof(short));
